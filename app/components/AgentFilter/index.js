...
class AgentFilter extends React.Component {
    ...
    render() {
        const { agents, fetchAgents, zones, fetchZones, regions, openPopup, closeFilters, formData, handleAgentForm, submitAgentFilter, organizations, checkValidation } = this.props;
            ...
            >
                <SafeAreaView style={[AppStyles.mb1, { backgroundColor: '#e7ecf0' }]}>
                    <ScrollView>
                        ...
                        <View style={styles.pad5}>
                            <PickerComponent clearOnChange={true} selectedItem={_.clone(formData.region)} onValueChange={(text) => { handleAgentForm(text, 'region'); fetchZones(text, 'agent') }} data={regions} placeholder='Region' />
                            {
