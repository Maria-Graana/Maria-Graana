...
const AttachmentTile = (props) => {
  const { data, deleteComment, property, deleteCommentCheck = true } = props
  return (
    <View style={styles.mainContainer}>
      ...
      {!deleteCommentCheck ? (
        <View style={styles.horizontalContainer}>
          <Text style={styles.userStyle}>{data.user}</Text>
        </View>
      ) : null}
      <View style={styles.horizontalContainer}>
      ...
      {property ? (
        <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
          <Text style={[styles.dateTimeStyle, { color: AppStyles.colors.primaryColor }]}>
            {data.title && data.title.toLocaleUpperCase()}
          </Text>
          <Text style={styles.dateTimeStyle}>
            {moment(data.createdAt).format('hh:mm A, MMMM DD, YYYY')}
          </Text>
        </View>
      ) : (
        <Text style={styles.dateTimeStyle}>
