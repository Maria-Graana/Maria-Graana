...
class MapContainer extends Component {
  ...
  fetchPlotData = async (propsure_id) => {
    ...
    axios.get(`${config.mapUrl}plot/${propsure_id}?secure=true`).then((res) => {
      const plot = res.data
      let data = plot[0]
      if (data.city) {
      ...
      if (data.phase_sector) {
        this.fetchPhaseSectorById(data.phase_sector.id)
      }
      this.setState({
        chosen_plot: plot[0],
        loading_plots: false,
  ...
  setPlotMarkers = (plots) => {
    const plot_markers = plots.map((plot) => ({
      ...plot,
      marker: this.getCentroid(JSON.parse(plot.geoData)),
    }))
    this.setState({
      plot_markers,
    })
                ...
                <ScrollView style={{ flex: 1 }}>
                  {cities ? (
                    <FlatList
                      data={cities}
                      renderItem={({ item }) => (
                        <CityRenderItem item={item} onCitySelect={this.onCitySelect} />
                      )}
                      keyExtractor={(item) => item.id}
                    />
                  ) : (
                    <ActivityIndicator color={'#0F73EE'} size={'large'} />
                  )}
                </ScrollView>
                <Button
                  title="Close"
              ...
              >
                <View style={{ padding: 10, marginTop: 46 }}>
                  ...
                  <TextInput
                    ...
                    onChangeText={(text) => {
                      this.onHousingSchemeSearch(text)
                    }}
                    autoCorrect={false}
            ...
            {!!chosen_plot && (
              <View style={styles.footer}>
                ...
                >
                  ...
                  <View style={styles.legalStatusStyle}>
                ...
                <Text style={styles.plotInfoStyle}>
                  {chosen_plot.housing_scheme ? chosen_plot.housing_scheme.housing_scheme_name : ''}
                  /{chosen_plot.phase_sector ? chosen_plot.phase_sector.phase_sector_name : ''},{' '}
