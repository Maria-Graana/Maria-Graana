...
class CMReport extends React.Component {
    ...
    axios
      ...
      .then((res) => {
        ...
        res &&
          res.data.rows.length &&
          res.data.rows.map((item, index) => {
            return agents.push({ value: item.id, name: item.firstName + ' ' + item.lastName })
          })
  ...
  regionUrl = () => {
    ...
    if (filterLabel === 'Quarterly') {
      ...
      url = `/api/leads/project/report?scope=region&q=${
        regionFormData.region
      }&timePeriod=${filterLabel.toLocaleLowerCase()}&fromDate=${selectedYear}-${
        quarter.fromDate
  ...
  organizationUrl = () => {
    ...
    if (filterLabel === 'Daily') {
      let dailyDate = moment(new Date(selectedDate)).format('MM-DD-YYYY')
      url = `/api/leads/project/report?scope=organization&q=${selectedOrganization}&timePeriod=${filterLabel.toLocaleLowerCase()}&fromDate=${dailyDate}`
    }
    if (filterLabel === 'Yearly')
    ...
    if (filterLabel === 'Weekly')
      url = `/api/leads/project/report?scope=organization&q=${selectedOrganization}&timePeriod=${filterLabel.toLocaleLowerCase()}&fromDate=${startWeek}&toDate=${endWeek}`
    if (filterLabel === 'Quarterly') {
      let newQaurter = this.setDefaultQuarter()
      let quarter = _.find(quarters, function (item) {
        return item.value === newQaurter
      ...
      url = `/api/leads/project/report?scope=organization&q=${selectedOrganization}&timePeriod=${filterLabel.toLocaleLowerCase()}&fromDate=${selectedYear}-${
        quarter.fromDate
      }&toDate=${selectedYear}-${quarter.toDate}`
    }
  ...
  checkDate = () => {
    ...
    if (filterLabel === 'Quarterly') {
      ...
      this.setState(
        { selectedDate: quarter.name + ', ' + selectedYear, selectedQuarter: newQaurter },
        () => {
          this.callReportApi()
        }
      )
          ...
          <ScrollView style={styles.scrollContainer}>
            <View style={{}}>
              {filterLabel === 'Daily' ? (
                ...
                <RectangleContainer
                  targetPercent={60}
                  targetNumber={dashBoardData.revenue}
                  totalTarget={dashBoardData.totalTarget}
                />
              )}
