...
class RCMReport extends React.Component {
    ...
    if (user.subRole === "regional_head") {
      if ("region" in user && user.region) {
        ...
        this.setState(
          {
            organizations: [
              { value: user.organizationId, name: user.organizationName },
            ],
            regionFormData,
            lastLabel: "Region",
            footerLabel: "Region",
  ...
  fetchZones = (value, check) => {
    ...
    if (check === "zone")
      org = this.organizationName(zoneFormData.organization);
    if (check === "agent")
      org = this.organizationName(agentFormData.organization);
    ...
    if (
      ...
      axios
        .get(
          `/api/user/teams?status=true&organizationId=${org.value}&all=true&regionId=${value}`
        )
        .then((res) => {
          let zones = [];
          res &&
  ...
  checkDate = () => {
    ...
    if (filterLabel === "Monthly")
      this.setState(
        { selectedDate: months[selectedMonth - 1] + " " + selectedYear },
        () => {
          this.callReportApi();
        }
      );
  ...
  openFilter = () => {
    ...
    const { user } = this.props;
    if (footerLabel === "Region" && user.role !== "sub_admin 1")
      this.openRegionFilter();
        ...
        <View style={styles.buttonsContainer}>
          <View style={styles.btnView}>
            <ReportFilterButton
              ...
              textPropStyle={
                filterLabel === "Daily" ? { color: "#ffffff" } : null
              }
            />
            <ReportFilterButton
              label="Weekly"
              ...
              textPropStyle={
                filterLabel === "Weekly" ? { color: "#ffffff" } : null
              }
            />
            <ReportFilterButton
              label="Monthly"
              selectedFilterButton={this.selectedFilterButton}
              buttonStyle={
                filterLabel === "Monthly" ? styles.selectedBtn : null
              }
              textPropStyle={
                filterLabel === "Monthly" ? { color: "#ffffff" } : null
              }
            />
            <ReportFilterButton
              label="Quarterly"
              selectedFilterButton={this.selectedFilterButton}
              buttonStyle={
                filterLabel === "Quarterly" ? styles.selectedBtn : null
              }
              textPropStyle={
                filterLabel === "Quarterly" ? { color: "#ffffff" } : null
              }
            />
            <ReportFilterButton
              label="Yearly"
              ...
              textPropStyle={
                filterLabel === "Yearly" ? { color: "#ffffff" } : null
              }
            />
          </View>
        </View>
